...
def create_model_data_dict(rts_gmlc_dir, begin_time, end_time, simulation="DAY_AHEAD", t0_state=None):
    ...
    # if simulation == "REAL_TIME" and load_timeseries_spec.DataFile == os.path.join(base_dir, '..',
    #                                                                                'timeseries_data_files', 'Load',
    #                                                                                'REAL_TIME_regional_Load.csv'):
    #     load_timeseries_spec = TimeSeriesPointer(load_timeseries_spec.Object, load_timeseries_spec.Simulation,
    #                                              load_timeseries_spec.Parameter,
    #                                              os.path.join(base_dir, '..', 'timeseries_data_files', 'Load',
    #                                                           'REAL_TIME_regional_load.csv'))
    ...
    if t0_state is None:
        unit_on_time_df = pd.read_csv(
            os.path.join(base_dir, '..', 'FormattedData', 'PLEXOS', 'PLEXOS_Solution', 'DAY_AHEAD Solution Files',
                         'noTX', 'on_time_7.12.csv'),
            header=0,
            sep=",")
...
def _create_rtsgmlc_skeleton(rts_gmlc_dir):
    ...
    for idx, row in gen_df.iterrows():
        ...
        if row['Fuel'] in ['Storage']:
            ...
            F_COEFF = [(x[i], round(f[i], 2)) for i in range(4) if
                       (((i == 0) or (x[i - 1], f[i - 1]) != (x[i], f[i])) and (x[i], f[i]) != (0., 0.))]
            if F_COEFF == []:
                F_COEFF = [(PMAX, 0.0)]
